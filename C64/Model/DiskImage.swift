...
extension DiskImage {
    ...
    func readDirectory() -> Directory? {
        ...
        return Directory(diskNamePETASCII: name, diskIdPETASCII: diskId, freeBlocks: freeBlocks, entries: entries)
    }
...
struct DxxImage : DiskImage {
    ...
    func getBlock(track: UInt8, sector: UInt8) -> Data? {
        guard let trackOffset = getBlockOffset(track: track, sector: sector) else { return nil }
        let offset = trackOffset + Int(sector)
    ...
    func getBlockOffset(track: UInt8, sector: UInt8) -> Int? {
        ...
        return layout.getOffset(track: track)
    }
