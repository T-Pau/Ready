        ...
        current_image->counter = (1000 - datasette_counter_offset +
                                (DS_G *
                                        (sqrt((current_image->cycle_counter
                                               ...
                                               * ds_c1) + ds_c2) - ds_c3)));
        current_image->counter -= floor(current_image->counter / 1000) * 1000;
...
void datasette_shutdown(void) {
    set_datasette_enable(0, NULL);
    alarm_destroy(datasette_alarm);
    datasette_alarm = NULL;
    datasette_alarm_pending = 0;
    datasette_long_gap_pending = 0;
}
