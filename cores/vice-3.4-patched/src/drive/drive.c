...
void drive_shutdown(void)
{
    ...
    for (dnr = 0; dnr < DRIVE_NUM; dnr++) {
        ...
        if (drive_context[dnr]->drive->gcr) {
            ...
            drive_context[dnr]->drive->gcr = NULL;
        }
...
void drive_set_initial_disk_id(unsigned int dnr, const uint8_t *id) {
    drive_context[dnr]->drive->initial_disk_id[0] = id[0];
    drive_context[dnr]->drive->initial_disk_id[1] = id[1];
}
