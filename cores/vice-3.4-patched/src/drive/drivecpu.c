...
inline static uint32_t drive_trap_handler(drive_context_t *drv)
{
    if (MOS6510_REGS_GET_PC(&(drv->cpu->cpu_regs)) == (uint16_t)drv->drive->trap) {
        ...
        if (drv->drive->initial_disk_id[0] != 0 || drv->drive->initial_disk_id[1] != 0) {
            drv->drive->drive_ram[0x12] = drv->drive->initial_disk_id[0];
            drv->drive->drive_ram[0x13] = drv->drive->initial_disk_id[1];
            drv->drive->initial_disk_id[0] = 0;
            drv->drive->initial_disk_id[1] = 0;
        }
