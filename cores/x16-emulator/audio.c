...
#else
    platform_audio_init(desired.freq, desired.samples, desired.channels);
...
#else
            uint16_t buf[2 * SAMPLES_PER_BUFFER];
            size_t length = sizeof(buf) / sizeof(buf[0]);
            
            if (platform_audio_available() >= length) {
                for (int i = 0; i < 2 * SAMPLES_PER_BUFFER; i++) {
                    buf[i] = ((int)psg_buf[i] + (int)pcm_buf[i] + (int)ym_buf[i]) / 3;
                }
                platform_audio_write(buf, length);
            }
#endif
		}
	}
